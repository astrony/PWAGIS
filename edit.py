...
def change_dataType(self, result_df):
    """
    Convert data types for specific layer attributes while handling null values appropriately.
    
    This function processes different layer types (FIREHYDRANT, VALVE, PIPE, etc.) and converts
    their respective attributes to proper integer types while safely handling null/None values.
    
    Args:
        self: The class instance containing layer and interface references
        result_df: DataFrame containing the feature data to be processed
        
    Returns:
        DataFrame: Modified DataFrame with corrected data types
    """
    ...
    if active_layer_name[1] == 'FIREHYDRANT':
        # Convert sizeId and statusId to integer, handling null values
        new_df['sizeId'] = new_df['sizeId'].astype('Int64')  # Nullable integer type
        new_df['statusId'] = new_df['statusId'].astype('Int64')  # Nullable integer type
        
    elif active_layer_name[1] == 'VALVE':
        # Convert basic valve attributes to integer
        new_df['typeId'] = new_df['typeId'].astype('Int64')
        new_df['sizeId'] = new_df['sizeId'].astype('Int64')
        new_df['statusId'] = new_df['statusId'].astype('Int64')
        
        # Handle functionId with proper null checking
        # Check if functionId column exists and has non-null values
        if 'functionId' in new_df.columns:
            # Use pandas nullable integer type to preserve null values
            new_df['functionId'] = new_df['functionId'].astype('Int64')
        
    elif active_layer_name[1] == 'PIPE':
        # Convert pipe-specific attributes to appropriate types
        new_df['productId'] = new_df['productId'].astype('Int64')
        new_df['functionId'] = new_df['functionId'].astype('Int64')
        new_df['layingId'] = new_df['layingId'].astype('Int64')
        
        # Handle optional attributes that might be null
        if 'classId' in new_df.columns:
            new_df['classId'] = new_df['classId'].astype('Int64')
        if 'gradeId' in new_df.columns:
            new_df['gradeId'] = new_df['gradeId'].astype('str')
        if 'sizeId' in new_df.columns:
            new_df['sizeId'] = new_df['sizeId'].astype('Int64')
            
    elif active_layer_name[1] == 'BLDG':
        # Handle building-specific attributes
        if 'useStatusId' in new_df.columns:
            new_df['useStatusId'] = new_df['useStatusId'].astype('Int64')
        if 'useTypeId' in new_df.columns:
            new_df['useTypeId'] = new_df['useTypeId'].astype('Int64')
        if 'buildingTypeId' in new_df.columns:
            new_df['buildingTypeId'] = new_df['buildingTypeId'].astype('Int64')
